function [MSE,minLambda,Lambda,S,W,params] = exp_cvxval_synth(varargin)
...
addOptional(p, 'saveName', 'results/results_exp_cvxval_synth');
...
for r = 1:p.Results.nR
    ...
    for s = 1:nS
        ...
        [Xy_p,Xy_n,Zy_p,Zy_n] = gen_cvshift('theta_Xy0', params.X_yn,'theta_Xy1', ...
            params.X_yp, 'N', p.Results.N, 'M', p.Results.M, 'pZ', pZ, ...
            'ub', 1./sqrt(2*pi*S(s)), 'xl', p.Results.domainLimits, ...
            'zl', p.Results.domainLimits);
        
        % Sample training data
        Vy_n = sampleDist(pX_yn, 1./sqrt(2*pi), round(p.Results.N.*pyn), p.Results.domainLimits, false);
        Vy_p = sampleDist(pX_yp, 1./sqrt(2*pi), round(p.Results.N.*pyp), p.Results.domainLimits, false);
        ...
        switch lower(p.Results.iwe)
            case 'none'
                W{s,r} = ones(1,size(V,1));
            case 'true'
                W{s,r} = pZ(V)./pX(V);
            case 'gauss'
                W{s,r} = iwe_Gauss(V',Z');
            case 'kmm'
                W{s,r} = iwe_KMM(V',Z',0,realmax);
            case 'kliep'
                W{s,r} = iwe_KLIEP(V,Z,0,realmax);
            case 'nnew'
                W{s,r} = iwe_NNeW(V,Z,0,realmax, 'Laplace', 1);
            otherwise
        ...
        [~,minl.V(s,r)] = min(MSE.V(s,:,r));
        [~,minl.W(s,r)] = min(MSE.W(s,:,r));
        [~,minl.Z(s,r)] = min(MSE.Z(s,:,r));
    end
end
